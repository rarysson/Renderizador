...
Renderizador::Renderizador(const Cena& cen, const Camera& cam, const Imagem& i, int n_a, int p) :
	cena {cen}, camera {cam}, img {i}, numero_amostras {n_a}, profundidade_dispersao {p} { }
...
void Renderizador::renderizar_cena() {
	for (int j = 0; j < img.altura; ++j) {
		std::vector<Cor> v;
